...
%
%addpath = '/home/tanvirdell3/Downloads/caserel-master/Publically available Dataset/Extracted frames for Segementation/Control/hc01_spectralis_macula_v1_s1_R_frame_019.png_octSegmentation.mat';
excelCompiled = {};
excelLumped = {};
...
for i = 2:numel(layersToAnalyze)
    ...
    excel = [excel; {strcat( [ layersToAnalyze{i-1} ' - ' layersToAnalyze{i}] ),...
        mean(layerCompile(secondLayerInd).x-layerCompile(firstLayerInd).x),...
        std(layerCompile(secondLayerInd).x-layerCompile(firstLayerInd).x)}];
end
